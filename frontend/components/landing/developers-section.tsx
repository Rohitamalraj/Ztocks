...
const [tier, expiry] = await client.readContract({
  ...
  {
    label: "Open Position",
    code: `import { useVault } from "@/hooks/use-vault"
...
await vault.openPosition(
  ...
  {
    label: "ZK Proof",
    code: `import { useZkIdentity } from "@/hooks/use-zk-identity"

const identity = useZkIdentity()

// 1) fetch oracle credential
// 2) generate Groth16 proof in-browser
// 3) submit to ZKVerifier on Sepolia
await identity.startVerification()`,
  },
];

const features = [
  { title: "TypeScript native", description: "Full type safety with auto-generated contract types." },
  { title: "Sepolia-ready", description: "Runs on Sepolia with viem/wagmi + RainbowKit." },
  { title: "Circom circuits", description: "Pre-built ZK circuits for tier verification." },
  { title: "Atomic enforcement", description: "Tier cap enforced at the EVM level, no backend." },
];
...
export function DevelopersSection() {
  ...
  return (
    <section id="protocol" ref={sectionRef} className="relative py-24 lg:py-32 overflow-hidden">
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-start">
          <div className={`transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            ...
            <p className="text-xl text-muted-foreground mb-12 leading-relaxed">
              Every component of Ztocks is modular and open. The zk verifier,
              vault, and oracle adapter are each independently deployable and
              auditable.
            </p>
