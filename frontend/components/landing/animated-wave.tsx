...
export function AnimatedWave() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([e]) => { activeRef.current = e.isIntersecting; }, { threshold: 0.05 });
    observer.observe(canvas);

    const render = (timestamp: number) => {
      if (!activeRef.current) { frameRef.current = requestAnimationFrame(render); return; }
      if (timestamp - lastTimeRef.current < 50) { frameRef.current = requestAnimationFrame(render); return; }
